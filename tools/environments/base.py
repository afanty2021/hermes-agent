...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(
        ...
        # Fork: adaptive idle timeout (seconds) — a command that produces no
        # output for this long is killed even before the hard ``timeout``
        # (silent long-runners like git clone hang forever otherwise;
        # terminal_tool adds --progress so busy commands keep feeding this).
        # 0/negative disables. See hermes_cli/config_defaults.py.
        try:
            from hermes_cli.config import load_config
            _idle_timeout = load_config().get("terminal", {}).get("idle_timeout", 60)
            if _idle_timeout <= 0:
        ...
        try:
            ...
            while proc.poll() is None:
                ...
                if yield_handler is not None and consume_yield(watch_interrupt_tid):
                    ...
                    drain_stop.clear()
                    drain_thread = _start_drain_thread(proc, output, drain_stop)
                # Fork: track output activity, then honor idle timeout alongside the hard deadline.
                _current_output_count = output.total_chars
                if _current_output_count > _last_output_count:
                    _last_output_activity = time.monotonic()
                    _last_output_count = _current_output_count
                _poll_now = time.monotonic()
                _hard_timeout = _poll_now > deadline
                _idle_timeout_exceeded = (
                    _idle_timeout is not None and
                    _poll_now - _last_output_activity > _idle_timeout
                )
                if _hard_timeout or _idle_timeout_exceeded:
                    ...
                    if _idle_timeout_exceeded and not _hard_timeout:
                        rendered = output.render(
                            suffix=f"\n[Command timed out after {_poll_now - _activity_state['start']:.0f}s (no output for {_idle_timeout}s)]")
                    else:
