...
def _record_call_outcome(server_name: str, result) -> Any:
    """Breaker bookkeeping (fork): ANY completed RPC — success payload or error payload — is proof
    the transport answered, so it RESETS the breaker. (Upstream #10447/#11113 burn a strike on
    error payloads and only fix the wording; this divergence is pinned by
...
async def _call_tool_racing_stdio_death(server, server_name: str, tool_name: str, args: dict,
    ...
    _call_coro = server.session.call_tool(tool_name, arguments=args, meta=session_meta)
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        # this thread's contextvars. Reading the session vars inside _call()
        # would miss the gateway-bound values. A plain local captured by the
        # closures carries the *value*, which is safe across threads.
        session_meta = _core._build_session_meta()
        ...
        async def _call():
            async with server._rpc_lock, _track_inflight_rpc(server, server_name, op):
                ...
                try:
                    # fork: meta carries the session identity (wecom session-level binding),
                    # captured outside the MCP loop thread (see above).
        ...
        def _on_failure(exc):
            if _core._is_application_level_mcp_error(exc):
                # Server answered with a JSON-RPC error (e.g. -32602 from an
                # identity/argument guard) — demonstrably reachable. Reset
