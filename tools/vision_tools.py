...
async def vision_analyze_tool(
    ...
    async def stage(prompt: str, debug_call_data: dict, temp_paths: list) -> tuple:
        ...
        # Fork: proactive resize gate — mid-size compressed photos time out the
        # aux LLM at full resolution while sitting far under the hard cap; the
        # byte budget is the effective lever (see _PROACTIVE_AUX_RESIZE_* notes).
        ...
        if len(image_data_url) > _MAX_BASE64_BYTES:
            image_data_url = await _resize_prepared(
                prepared, _scale_info, max_dimension=_PROACTIVE_AUX_RESIZE_DIMENSION_CAP)
            if len(image_data_url) > _MAX_BASE64_BYTES:
