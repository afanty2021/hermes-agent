...
# computer_use controls a real desktop — concurrent invocations would race on
# the same screen/mouse/keyboard, so it must run alone in its segment (fork
# replay; lost once before to an upstream set refactor).
_NEVER_PARALLEL_TOOLS = frozenset({"clarify", "computer_use", "manage_connections"})
