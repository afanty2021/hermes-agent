...
def auto_title_session(
    ...
    ``secret_scope`` / ``hermes_home`` republish the caller's profile boundary on this thread: a
    ``threading.Thread`` starts with a fresh context, so the contextvars installed by the gateway's
    ``_profile_runtime_scope`` (``agent.secret_scope`` + the Hermes home override) are invisible here —
    under multiplexing the title LLM call would fail closed on ``get_secret``, and home-dependent reads
    would resolve the *default* profile's ``.env``/config. ``maybe_auto_title`` captures both while the
    turn's scope is active; None (single-profile callers) installs nothing.
    """
    try:
...
def maybe_auto_title(
    ...
    # auto_title_session itself uses for the conversation and accounting
