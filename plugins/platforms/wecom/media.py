...
class WeComMediaMixin:
    ...
    async def _cache_media(
        ...
        if kind == "image":
            # Fork (a2efd297d2): for images the bytes' magic is authoritative — the CDN's
            # content-type and the URL suffix are both guesses (same rule as the base64 branch,
            # which reads the ext straight off the magic).
    ...
    async def _store_media(self, kind, raw, ext, image_mime, filename, doc_mime, origin, *, chat_id: Optional[str] = None) -> Optional[Tuple[str, str]]:
        ...
        whitelist and the downstream vision models both reject HEIF, so pre-20260913-merge the
        bytes were silently dropped at every branch. Non-image/broken bytes keep the ValueError
        reject channel (warning + None), never a bogus image handed downstream."""
        if kind != "image":
            return await cache_document_from_bytes_async(raw, filename), doc_mime
        try:
            if ext == ".heic":
                # CPU-heavy (12MP measured ~446ms/image) — run in the thread pool so the event
                # loop never blocks.
    ...
    def _convert_heic_to_jpeg(data: bytes) -> bytes:
        """iPhone HEIC (HEIF container) -> JPEG: after transcoding bytes/ext/mime all ride the
        existing JPEG path. Depends on pillow-heif (lazy import; absent -> ImportError ->
        ValueError reject, non-fatal). All failures raise ValueError to reuse the
        non-image-reject channel."""
        try:
            ...
            with Image.open(BytesIO(data)) as img:
                oriented = ImageOps.exif_transpose(img) or img  # iPhone portrait EXIF orientation
                out = BytesIO()
    ...
    def _detect_image_ext(data: bytes) -> str:
        ...
        # HEIF/HEIC container (ISO BMFF: bytes[4:8]==ftyp, major brand 8:12).
        # iPhone camera default; brand set covers Apple stills (heic/heix/heim/heis/mif1),
        # other-vendor HEVC containers (hevc/hevx/hevm/hevs/msf1) and bare heif. AVIF not
        # included (needs pillow-avif-plugin; never seen in WeCom traffic — add on first sight).
    ...
    def _mime_for_ext(ext: str, fallback: str = "application/octet-stream") -> str:
        # Hardcoded table instead of mimetypes.types_map: types_map statically lacks .webp when
        # the process never called mimetypes.init() (and never called any guess_*), so a direct
        # read bounced webp images to the fallback octet-stream -> DOCUMENT misclassification
        # (fork review I1). Note: _detect_image_ext can also output .heic, but that is transient —
        # the only caller _store_media transcodes to .jpg before consulting this table; .heic
        # deliberately stays out of the table (an unexpected direct hit falls back to DOCUMENT,
        # a visible rejection, rather than lying with image/heic).
        ext_mime = {
            ".png": "image/png",
            ".jpg": "image/jpeg",
            ".gif": "image/gif",
    ...
    def _guess_filename(url: str, content_disposition: Optional[str], content_type: str) -> str:
        ...
        if match:
            return unquote(match.group(1))  # zops patch 0005: CDN names arrive URL-encoded (中文课件)
        name = Path(unquote(urlparse(url).path)).name or "document"  # zops patch 0005
