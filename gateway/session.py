...
def build_channel_continuity_note(entry: "SessionEntry", source: SessionSource) -> Optional[str]:
    ...
    Fork: WeCom DMs are the same shape for teachers (2026-09-07 incident: bot recommended
    videos at 00:22, daily 04:00 reset, and the teacher's 10:41 "add that video to my plan"
    hit a fresh history=0 session where the bot disclaimed any knowledge). No LLM calls, no
    extra lookups — prev_session_id is already known on the entry.
    """
    ...
    if source.platform is Platform.WECOM:
        where = "chat"  # fork: WeCom DM wording (no thread/channel concept in WeCom)
    else:
        ...
        with self._lock:
            ...
            if stale_hit or reset_reason:
                ...
                if reset_reason:
                    ...
                    # finalizer ran while sessions.json still held the old id). Recovery may still
                    # reopen a recoverable end_reason (transcript preserved — these flags are then
                    # ignored); if it cannot, the fresh session must carry the same continuity hint
                    # an auto-reset gives — otherwise the agent answers with zero knowledge of the
                    # prior same-chat session (2026-09-09 ggtms: overnight session_reset; morning
                    # follow-up got no hint and re-recommended the prior night's video).
                    decision.schedule_reset("stale_recovery", entry, entry.last_prompt_tokens > 0)
