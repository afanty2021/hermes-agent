...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    # （20260913 合并核查：上游 6489 提交内未见注册侧修复，补丁保留。）
    try:
...
_AUTO_RESET_CONTEXT_NOTES = {
    ...
    # fork (47a901e8e1): stale rebuild with no pending reset decision — the fresh session must
    # tell the agent the prior same-chat session exists (session_search). Never user-notified:
    # the end was already user-facing when the finalizer ran; the note suffices.
    "stale_recovery": "[System note: The user's previous session had already ended when this conversation started, so a fresh session was created. This is a fresh conversation with no prior context.]",
}
